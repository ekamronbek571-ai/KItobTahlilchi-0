...
async def analyze_image_with_openrouter(image_data: bytes, mime_type: str = "image/jpeg") -> str:
    """OpenRouter Vision API orqali rasmni tahlil qilish"""
    image_b64 = base64.standard_b64encode(image_data).decode("utf-8")

    headers = {
        "Authorization": f"Bearer {OPENROUTER_API_KEY}",
        "Content-Type": "application/json",
        "HTTP-Referer": "https://kitobtahlilchi-0.onrender.com",
...
async def get_recommendation_from_openrouter(user_request: str, books_list: str) -> str:
    """OpenRouter orqali kitob tavsiya olish"""
    headers = {
        "Authorization": f"Bearer {OPENROUTER_API_KEY}",
        "Content-Type": "application/json",
        "HTTP-Referer": "https://kitobtahlilchi-0.onrender.com",
...
async def handle_text(message: Message, state: FSMContext):
    ...
    if current_state == UserStates.waiting_recommendation_request:
        ...
        try:
            recommendation = await get_recommendation_from_openrouter(message.text, books_list)
            await message.answer(
                f"💡 *Tavsiya:*\n\n{recommendation}",
                parse_mode="Markdown",
                reply_markup=main_menu(message.from_user.id),
...
async def search_by_image(call: CallbackQuery):
    await call.message.edit_text(
        "📸 Kitob rasmini yuboring.\n\nAI rasmni tahlil qilib kitobni aniqlaydi!"
    )
...
async def handle_photo(message: Message, state: FSMContext):
    ...
    try:
        ...
        detected_name = await analyze_image_with_openrouter(image_data)
...
async def list_books(call: CallbackQuery):
    ...
    buttons = []
    for book in books:
        buttons.append([InlineKeyboardButton(text=f"📖 {book['name']}", callback_data=f"book_{book['id']}")])
